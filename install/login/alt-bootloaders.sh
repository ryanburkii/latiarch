...
if ! command -v limine &>/dev/null; then
  ...
  if [ -d "/boot/loader/entries" ]; then # systemd-boot
    ...
    if ! grep -q splash /etc/cmdline.d/*.conf; then
      # Need splash, create the latiarch file
      echo "splash" | sudo tee -a /etc/cmdline.d/latiarch.conf
    fi
    if ! grep -q quiet /etc/cmdline.d/*.conf; then
      # Need quiet, create or append the latiarch file
      echo "quiet" | sudo tee -a /etc/cmdline.d/latiarch.conf
    fi
