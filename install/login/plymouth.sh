...
if [ ! -f /etc/systemd/system/latiarch-seamless-login.service ]; then
  cat <<EOF | sudo tee /etc/systemd/system/latiarch-seamless-login.service
[Unit]
Description=LatiArch Seamless Auto-Login
Documentation=https://github.com/ryanburkii/latiarch
...
# Enable latiarch-seamless-login.service only if not already enabled
if ! systemctl is-enabled latiarch-seamless-login.service | grep -q enabled; then
  sudo systemctl enable latiarch-seamless-login.service
fi
