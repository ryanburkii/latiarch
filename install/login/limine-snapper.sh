...
#timeout: 3
default_entry: 2
interface_branding: LatiArch Bootloader
...
EOF
  ...
  if [[ -z ${LATIARCH_CHROOT_INSTALL:-} ]]; then
...
if [[ -n $EFI ]] && efibootmgr &>/dev/null &&
  ...
  uki_file=$(find /boot/EFI/Linux/ -name "latiarch*.efi" -printf "%f\n" 2>/dev/null | head -1)
