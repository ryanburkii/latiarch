...
# Directs user to LatiArch Discord
QR_CODE='
█▀▀▀▀▀█ ▄ ▄ ▀▄▄▄█ █▀▀▀▀▀█
█ ███ █ ▄▄▄▄▀▄▀▄▀ █ ███ █
█ ▀▀▀ █ ▄█  ▄█▄▄▀ █ ▀▀▀ █
...
show_log_tail() {
  if [[ -f $LATIARCH_INSTALL_LOG_FILE ]]; then
    local log_lines=$(($TERM_HEIGHT - $LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$LATIARCH_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "LatiArch installation stopped!"
  show_log_tail
  ...
  while true; do
    ...
    if [[ -n ${LATIARCH_ONLINE_INSTALL:-} ]]; then
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/latiarch/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$LATIARCH_INSTALL_LOG_FILE"
      else
        tail "$LATIARCH_INSTALL_LOG_FILE"
      fi
      ;;
    "Upload log for support")
      latiarch-upload-install-log
      ;;
    "Exit" | "")
      exit 1
      ;;
